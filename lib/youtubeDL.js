...
const downloadVideo = (id, options = {}) => {
  return new Q((yes, no) => {
    ...
    const child = spawn(`${youtubeDLPath}`, compact([
...
const getManifest = (id, options = { youtubeDLPath: "youtube-dl" }) => {
  return new Q((yes, no) => {
    ...
    const manifestUrl = compact(requested_formats.map(fmt => fmt.manifest_url))
...
module.exports = { downloadVideo, getManifest }
